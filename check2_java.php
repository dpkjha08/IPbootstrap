...
if($count==1){
    ...
    echo"<br>";
    echo "attempt is: ".$_SESSION['java_attempt']."/1";
    echo"<br>";
...
$python_attempt=$_SESSION['java_attempt'];
...
if ($python_attempt==1){
    ...
    mysqli_query($con,"UPDATE `signupdata` set `js_attempt`=1 where `email`='$email' ");
   mysqli_query($con,"UPDATE `signupdata` set `js_percent`= '$total' where `email`='$email' ");
   $result= mysqli_query($con,"SELECT * FROM `signupdata` WHERE `email`='$email' && `password`='$password'");
    if($count==1){
        $_SESSION['log']=1;
       $row=mysqli_fetch_row($result);
