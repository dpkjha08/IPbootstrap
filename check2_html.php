...
if($count==1){
    ...
    echo"<br>";
    echo "attempt is: ".$_SESSION['html_attempt']."/1";
    echo"<br>";
...
if ($python_attempt==1){
    ...
    mysqli_query($con,"UPDATE `signupdata` set `java_attempt`=1 where `email`='$email' ");
   mysqli_query($con,"UPDATE `signupdata` set `java_percent`= '$total' where `email`='$email' ");
   $result= mysqli_query($con,"SELECT * FROM `signupdata` WHERE `email`='$email' && `password`='$password'");
    if($count==1){
        $_SESSION['log']=1;
       $row=mysqli_fetch_row($result);
